  ...
  const VoucherList = () => {
    ...
    const handleSubmitForm = async (formData) => {
      try {
        ...
        if (isEdit) {
          await AdminService.updateVoucher(selectedVoucher._id, formData);
          setSnackbar({
            open: true,
    ...
    const handleDeleteConfirm = async () => {
      try {
        setLoading(true);
        await AdminService.deleteVoucher(deleteDialog.voucher._id);
    ...
    try {
      ...
      await AdminService.updateVoucher(voucher._id, {
        isActive: newStatus
      });

      setSnackbar({
        open: true,
        message: `Voucher ${newStatus ? 'activated' : 'deactivated'} successfully`,
        severity: 'success'
      });
    } catch (error) {
