...
const STATUS_CONFIG = {
  ...
  pending:  { label: "In Review",  color: "#d97706", bg: "#fef3c7", icon: <ClockCircleOutlined /> },
  approved: { label: "Published",  color: "#059669", bg: "#d1fae5", icon: <CheckCircleOutlined /> },
...
const BlogPreviewModal = ({ blog, open, onClose }) => {
  ...
  return (
    <Modal open={open} onCancel={onClose} footer={null} width={760}
      styles={{ body: { padding: 0 } }} style={{ borderRadius: 20, overflow: "hidden" }}>
      {blog.thumbnail && (
      ...
      <div style={{ padding: "28px 32px" }}>
        ...
        <h3 style={{ margin: "0 0 10px", fontSize: 22, fontWeight: 800, color: C.text }}>{blog.title}</h3>
        <Text style={{ color: C.textSub, fontSize: 14, display: "block", marginBottom: 16, lineHeight: 1.6 }}>{blog.summary}</Text>
        <div style={{ borderTop: `1px solid ${C.border}`, paddingTop: 16, marginBottom: 16, fontSize: 14, lineHeight: 1.8, color: C.text }}
...
const EditBlogModal = ({ blog, open, onClose, onSaved, categories }) => {
  ...
  return (
      ...
      title={
        <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
          ...
          {blog?.status === "rejected" && (
            <Tag style={{ borderRadius: 20, border: "none", background: "#fee2e2", color: "#dc2626", fontWeight: 700, fontSize: 11, marginLeft: 4 }}>
              Rejected
            </Tag>
          )}
        </div>
      ...
      footer={
        <Space>
          ...
          <Button icon={<SaveOutlined />} onClick={() => handleSave(false)} loading={saving}
            style={{ borderRadius: 10, fontWeight: 700 }}>Save Draft</Button>
          <Button type="primary" icon={<SendOutlined />} onClick={() => handleSave(true)} loading={saving}
            style={{ background: C.gradient, border: "none", borderRadius: 10, fontWeight: 700 }}>
            Submit for Review
          </Button>
        </Space>
      }
    >
      {blog?.rejectedReason && (
        <div style={{ background: C.redBg, border: "1px solid #fecaca", borderRadius: 10, padding: "10px 14px", marginBottom: 16, display: "flex", alignItems: "flex-start", gap: 8 }}>
          <ExclamationCircleOutlined style={{ color: C.red, marginTop: 2, flexShrink: 0 }} />
          <div>
            <Text style={{ color: C.red, fontWeight: 700, fontSize: 13, display: "block" }}>Rejection reason:</Text>
            <Text style={{ color: C.red, fontSize: 13 }}>{blog.rejectedReason}</Text>
          </div>
        </div>
      )}
      ...
      <Form form={form} layout="vertical" style={{ marginTop: 4 }}
        ...
        <Row gutter={16}>
          <Col span={16}>
            <Form.Item name="title" label="Title"
              rules={[{ required: true, message: "Title is required" }, { max: 255 }]}>
              <Input style={{ borderRadius: 8, fontSize: 15, fontWeight: 600 }} placeholder="Blog title..." />
            </Form.Item>
          </Col>
          <Col span={8}>
            <Form.Item name="category" label="Category"
              rules={[{ required: true, message: "Please select a category" }]}>
              <Select style={{ borderRadius: 8 }} placeholder="Select category"
        ...
        <Form.Item name="summary" label="Summary"
          rules={[{ required: true, message: "Summary is required" }, { max: 1000 }]}>
          <Input.TextArea rows={2} style={{ borderRadius: 8, resize: "none" }} placeholder="Short summary..." />
        </Form.Item>
        ...
        <Form.Item label="Cover Image">
          <div style={{ display: "flex", gap: 12, alignItems: "center", flexWrap: "wrap" }}>
            {thumbnail && (
              <div style={{ position: "relative", flexShrink: 0 }}>
                <img src={thumbnail} alt="thumb"
                  style={{ width: 130, height: 78, objectFit: "cover", borderRadius: 10, border: `1px solid ${C.border}` }}
                  onError={(e) => { e.target.style.display = "none"; }} />
                <button type="button" onClick={() => setThumbnail("")}
                  style={{ position: "absolute", top: -7, right: -7, width: 22, height: 22, borderRadius: "50%", background: C.red, border: "2px solid white", color: "white", fontSize: 11, cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center" }}>
                  ✕
                </button>
              </div>
            )}
            <Upload accept="image/*" showUploadList={false}
              ...
              <Button icon={uploadingThumb ? <LoadingOutlined /> : <UploadOutlined />} loading={uploadingThumb}
        ...
        <Form.Item
          ...
          required
        >
          <BlogTinyEditor height="55vh" placeholder="Write your blog content..." />
        </Form.Item>
      </Form>
    </Modal>
  );
...
const DeleteConfirmModal = ({ blog, open, onClose, onDeleted }) => {
  ...
  return (
    <Modal open={open} onCancel={onClose}
      title={<div style={{ display: "flex", alignItems: "center", gap: 10, color: C.red }}><ExclamationCircleOutlined /><span>Delete Blog Post</span></div>}
      footer={<Space><Button onClick={onClose}>Cancel</Button><Button danger type="primary" icon={<DeleteOutlined />} loading={loading} onClick={handleDelete}>Delete</Button></Space>}
      width={420}>
      <p style={{ color: C.textSub, fontSize: 14, lineHeight: 1.6, marginTop: 8 }}>
        Are you sure you want to delete <strong style={{ color: C.text }}>"{blog?.title}"</strong>?<br />
...
const InstructorBlogPage = () => {
  ...
  useEffect(() => { CourseService.getCategories().then(setCategories).catch(() => {}); }, []);
  useEffect(() => { loadBlogs(); }, [activeTab, pagination.current]);
  ...
  const loadBlogs = async () => {
    ...
    } catch { message.error("Failed to load blogs."); }
    finally { setLoading(false); }
  };
  ...
  const columns = [
    {
      title: "Blog Post", key: "blog",
      render: (_, r) => (
        <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
          {r.thumbnail
            ? <img src={r.thumbnail} alt="" style={{ width: 56, height: 42, objectFit: "cover", borderRadius: 8, flexShrink: 0 }} onError={(e) => { e.target.style.display = "none"; }} />
            : <div style={{ width: 56, height: 42, borderRadius: 8, background: C.primaryBg, display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}><FileTextOutlined style={{ color: C.primary }} /></div>
          }
          <div style={{ minWidth: 0 }}>
            <Text strong style={{ fontSize: 13, color: C.text, display: "block", marginBottom: 2 }} ellipsis={{ tooltip: r.title }}>{r.title}</Text>
            <Text style={{ fontSize: 12, color: C.textMuted }}>{r.summary?.slice(0, 72)}{r.summary?.length > 72 ? "…" : ""}</Text>
          </div>
        </div>
      ),
    },
    ...
    },
    {
      ...
      render: (_, r) => {
        ...
        return <Tag style={{ borderRadius: 20, border: "none", background: st.bg, color: st.color, fontWeight: 700, fontSize: 11 }}>{st.icon} {st.label}</Tag>;
      },
    },
    {
      title: "Created", key: "date", width: 110,
      render: (_, r) => <Text style={{ fontSize: 12, color: C.textSub }}>{new Date(r.createdAt).toLocaleDateString("en-GB")}</Text>,
    },
    {
        ...
        <Space size={4}>
          ...
          {r.status === "draft" && (
            <Tooltip title="Submit for Review">
              <Button type="text" icon={<SendOutlined />} size="small" style={{ color: C.mint }}
                loading={submittingId === r._id} onClick={() => handleSubmitForReview(r)} />
            </Tooltip>
          )}
          {(r.status === "draft" || r.status === "rejected") && (
            <Tooltip title="Delete">
              <Button type="text" icon={<DeleteOutlined />} size="small" style={{ color: C.red }} onClick={() => setDeleteBlog(r)} />
            </Tooltip>
          )}
        </Space>
      ),
  ...
  return (
    <motion.div variants={pageVariants} initial="initial" animate="animate" exit="exit">
      <div style={{ minHeight: "100vh", background: "#f9fafb", padding: "28px 28px 40px" }}>
        ...
        {/* Header */}
        <motion.div {...up(0)} style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", marginBottom: 24, gap: 16 }}>
          <div>
            ...
            <h2 style={{ margin: "0 0 4px", fontSize: 26, fontWeight: 900, background: C.gradient, WebkitBackgroundClip: "text", WebkitTextFillColor: "transparent" }}>
              My Blog Posts
            </h2>
          ...
          <Button type="primary" icon={<PlusOutlined />} size="large"
            ...
            style={{ borderRadius: 12, fontWeight: 700, background: C.gradient, border: "none", boxShadow: "0 4px 16px rgba(99,102,241,0.3)", height: 44, paddingInline: 24, flexShrink: 0 }}>
            New Blog Post
          </Button>
        </motion.div>
        ...
        <Row gutter={[14, 14]} style={{ marginBottom: 22 }}>
          ...
          <Col xs={12} sm={6}><StatCard label="Draft"      value={counts.draft}       color={C.textSub}  icon={<FileTextOutlined />}   delay={0.1} /></Col>
          <Col xs={12} sm={6}><StatCard label="In Review"  value={counts.pending}     color={C.amber}    icon={<ClockCircleOutlined />} delay={0.15} /></Col>
          <Col xs={12} sm={6}><StatCard label="Published"  value={counts.approved}    color={C.mint}     icon={<CheckCircleOutlined />} delay={0.2} /></Col>
        </Row>

        {/* Table */}
        <motion.div {...up(0.15)}>
          <Card bordered={false} style={card} bodyStyle={{ padding: 0 }}>
            <div style={{ display: "flex", alignItems: "center", gap: 10, flexWrap: "wrap", padding: "16px 20px", borderBottom: `1px solid ${C.border}` }}>
              <div style={{ display: "flex", gap: 6, flex: 1, flexWrap: "wrap" }}>
                {TABS.map((t) => (
                  <button key={t.key}
                    onClick={() => { setActiveTab(t.key); setPagination((p) => ({ ...p, current: 1 })); }}
                    style={{ padding: "5px 14px", borderRadius: 20, fontSize: 12, fontWeight: 700, border: "none", cursor: "pointer", background: activeTab === t.key ? C.primaryBg : "transparent", color: activeTab === t.key ? C.primary : C.textSub, outline: activeTab === t.key ? `1px solid ${C.primary}30` : "none", transition: "all 0.2s" }}>
                    {t.label}
                  </button>
                ))}
              </div>
              <Input prefix={<SearchOutlined style={{ color: C.textMuted }} />} placeholder="Search posts..."
                value={search} onChange={(e) => setSearch(e.target.value)} onPressEnter={loadBlogs}
            ...
            <Table columns={columns} dataSource={blogs} rowKey="_id" loading={loading}
              pagination={{
                ...
                showTotal: (total) => <Text style={{ fontSize: 12, color: C.textSub }}>{total} posts</Text>,
      ...
      <EditBlogModal blog={editBlog} open={!!editBlog} onClose={() => setEditBlog(null)} onSaved={loadBlogs} categories={categories} />
      <DeleteConfirmModal blog={deleteBlog} open={!!deleteBlog} onClose={() => setDeleteBlog(null)} onDeleted={loadBlogs} />
    </motion.div>
  );
};
