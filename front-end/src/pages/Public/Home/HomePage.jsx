...
const MILESTONES = [
  { icon: "compass", title: "Discover", sub: "Tech Discovery", desc: "Find your career direction and passion", route: ROUTES.COURSES },
  { icon: "book-open", title: "Learn", sub: "Digital Wisdom", desc: "Expert-led, deeply structured curriculum", route: ROUTES.COURSES },
  { icon: "code", title: "Build", sub: "Cyber Forge", desc: "Ship real projects inside the Lab", route: ROUTES.COURSES },
  { icon: "award", title: "Certify", sub: "Neon Credential", desc: "Prove your skills internationally", route: ROUTES.ABOUT },
];
  ...
  return (
    ...
    >
      ...
      <div style={{ position: "relative", height: 180, overflow: "hidden", flexShrink: 0 }}>
        ...
        />
        {/* gradient overlay */}
        <div style={{ position: "absolute", inset: 0, background: "linear-gradient(to bottom, transparent 45%, rgba(15,23,42,0.45) 100%)" }} />
        {/* Category badge */}
        {blog.category?.name && (
      ...
      {/* Content */}
      <div style={{ padding: "18px 20px 20px", display: "flex", flexDirection: "column", flex: 1, gap: 8 }}>
        {/* Title */}
        <h4 style={{
          margin: 0,
          fontSize: 15,
        ...
        <div style={{
          ...
          <div style={{ flex: 1, minWidth: 0 }}>
            <div style={{ fontSize: 11, fontWeight: 700, color: "var(--text-body)", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>
              {authorName}
            </div>
    ...
    <>
      ...
      <motion.div variants={pageVariants} initial="initial" animate="animate" exit="exit" style={{ position: "relative", zIndex: 1 }}>
        ...
        <section className="px-6 pt-24 pb-20 mx-auto text-center max-w-7xl">
          ...
          >
            World-class curriculum crafted by industry experts. Build real skills, earn globally recognized certificates, and accelerate your career.
          </motion.p>
          ...
          >
            ...
            >
              <Icon name="play" size={20} color="var(--color-secondary)" /> Browse Courses
            </GlassCard>
          </motion.div>

          {/* Dashboard Preview */}
          <motion.div
            initial={{ opacity: 0, y: 40 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.45 }}
              ...
              <div className="flex flex-col gap-8 md:flex-row">
                    ...
                    {SIDEBAR_ITEMS.map((item) => (
                      <button
                        ...
                        className={`w-full flex items-center gap-3 px-3 py-3 rounded-xl font-semibold text-sm transition-colors text-left ${
                          item.active ? "bg-white shadow-sm text-heading" : "text-muted hover:text-body hover:bg-white/40"
                        }`}
                      >
                        <Icon name={item.icon} size={18} color={item.active ? "var(--color-secondary)" : "currentColor"} />
                        {item.label}
                      </button>
                    ))}
                  </div>
                ...
                <div className="flex-1 text-left">
                  <div className="flex items-center justify-between mb-8">
                    <h3 className="text-3xl font-extrabold tracking-tight text-heading">Learning Hub</h3>
                    <div className="flex items-center gap-4">
                      <div className="px-4 py-2 text-xs font-bold border rounded-full bg-white/60 border-border">76% Complete</div>
                      <button
                  ...
                  <div className="grid grid-cols-1 gap-6 mb-8 md:grid-cols-3">
                    <div className="p-5 border bg-white/40 rounded-3xl border-white/60">
                      <span className="text-[10px] text-muted font-bold uppercase tracking-widest mb-3 block">Growth Rate</span>
                      <div className="flex items-end h-24 gap-2">
                        {[50, 65, 100, 75].map((h, i) => (
                          <div key={i} className="flex-1 rounded-xl"
                            style={{ height: `${h}%`, background: i === 2 ? "var(--gradient-brand)" : "var(--color-primary-bg)", opacity: i === 2 ? 1 : 0.6 }}
                          />
                        ))}
                      </div>
                    </div>
                    <div className="flex flex-col items-center justify-center p-5 border bg-white/40 rounded-3xl border-white/60">
                      <span className="text-[10px] text-muted font-bold uppercase tracking-widest mb-3 block">Overall Progress</span>
                      <div className="relative w-20 h-20">
                        <svg className="w-full h-full" style={{ transform: "rotate(-90deg)" }} viewBox="0 0 36 36">
                          <circle cx="18" cy="18" r="16" fill="none" stroke="#E2E8F0" strokeWidth="3" />
                          <circle cx="18" cy="18" r="16" fill="none" stroke="url(#pg)" strokeDasharray="76,100" strokeLinecap="round" strokeWidth="3" />
                          <defs>
                        ...
                        <div className="absolute inset-0 flex items-center justify-center text-lg font-bold text-heading">76%</div>
                      </div>
                    </div>
                    <div className="p-5 border bg-white/40 rounded-3xl border-white/60">
                      <span className="text-[10px] text-muted font-bold uppercase tracking-widest mb-3 block">Weekly Activity</span>
                      <div className="pt-2 space-y-3">
                        {[100, 65, 80].map((w, i) => (
                          <div key={i} className="h-2.5 bg-slate-100 rounded-full overflow-hidden">
                            <div className="h-full rounded-full" style={{ width: `${w}%`, background: i % 2 === 0 ? "var(--color-primary)" : "var(--color-secondary)", opacity: 0.7 }} />
                          </div>
                        ))}
                      </div>
                    </div>
                    ...
                    {upNext.length === 0 ? (
                      ...
                      <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
                        {upNext.map((course) => (
                          <button
                            key={course._id}
                            onClick={() => enrolledCourseIds.includes(course._id) ? navigate(`/student/learning/${course._id}`) : navigate(`/courses/${course._id}`)}
                          ...
                          >
                            <div className="flex items-center gap-4">
                              <div className="overflow-hidden w-11 h-11 rounded-xl shrink-0">
                                <img src={course.thumbnail || "https://images.unsplash.com/photo-1516321318423-f06f85e504b3?w=88&h=88&fit=crop"} alt={course.title} className="object-cover w-full h-full" />
                              </div>
                              <div>
                                <p className="text-xs font-bold tracking-tight uppercase text-heading line-clamp-1">{course.title}</p>
                                <p className="text-[9px] font-bold text-muted uppercase tracking-tighter mt-0.5">{course.category?.name} · {course.level}</p>
                              </div>
                            </div>
        ...
        {/* ══ LEARNING PATH ════════════════════════════════════════════════ */}
        <section className="relative px-6 py-16 mx-auto max-w-7xl" style={{ background: "radial-gradient(circle at 50% 50%,rgba(240,253,250,0.5) 0%,transparent 100%)" }}>
          <div className="mb-16 text-center">
            <h2 className="mb-3 text-4xl font-extrabold tracking-tight text-heading">Your Learning Journey</h2>
            <p className="font-medium text-muted">Four strategic milestones from beginner to certified professional</p>
          </div>
          <div className="relative min-h-[500px] flex items-center justify-center">
            <svg className="absolute z-0 w-full pointer-events-none" style={{ height: 500 }} preserveAspectRatio="none" viewBox="0 0 1200 500">
              <defs>
              ...
              <path className="opacity-60" d="M 150 400 C 300 400,350 150,450 150 C 550 150,650 375,750 375 C 850 375,950 150,1100 150" fill="none" stroke="url(#ribbonGrad)" strokeWidth="12" strokeLinecap="round" filter="url(#ribbonGlow)" />
              <path className="opacity-80" d="M 150 400 C 300 400,350 150,450 150 C 550 150,650 375,750 375 C 850 375,950 150,1100 150" fill="none" stroke="white" strokeWidth="2" strokeLinecap="round" strokeDasharray="2 10" />
            </svg>
            <div className="absolute inset-0 z-10">
              {[
                ...
                <motion.button
                  key={ms.title}
                  initial={{ opacity: 0, scale: 0.8 }} whileInView={{ opacity: 1, scale: 1 }} transition={{ delay }}
                ...
                >
                  <div className="w-40 h-28 glass-card rounded-[2rem] border-2 flex flex-col items-center justify-center p-4"
        ...
        <section className="px-6 py-16 mx-auto max-w-7xl">
          <div className="mb-12 text-center">
            <h2 className="mb-3 text-4xl font-extrabold tracking-tight text-heading">Next-gen Learning Tools</h2>
            <p className="font-medium text-muted">AI-powered and simulator-driven environments</p>
          </div>
          <div className="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-4">
            {TOOLS.map((tool, i) => (
              <motion.div key={tool.title} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.1 }}>
                <GlassCard className="p-8 rounded-[2.5rem] group hover:bg-white/80 transition-all duration-300 hover:border-primary/20 h-full cursor-pointer" onClick={() => navigate(tool.route)}>
                  <div className="flex items-center justify-center mb-6 w-14 h-14 rounded-2xl" style={{ background: tool.bg }}>
        ...
        <section className="px-6 py-16 mx-auto max-w-7xl">
          <div className="mb-12 text-center">
            <h2 className="mb-3 text-4xl font-extrabold tracking-tight text-heading">Featured Courses</h2>
          ...
          {!sectionsLoading && sectionsError && (
            <div className="py-16 text-center">
              <p className="mt-4 font-medium text-muted">Could not load courses. Please try again.</p>
          ...
          {!sectionsLoading && !sectionsError && (
            <div className="space-y-16">
              {categorySections.map((section, si) => (
                <motion.div key={section.category._id} initial={{ opacity: 0, y: 24 }} whileInView={{ opacity: 1, y: 0 }} transition={{ delay: si * 0.07 }}>
                  <div className="flex items-center justify-between mb-6">
                    <div className="flex items-center gap-3">
                      <div className="w-1.5 h-8 rounded-full" style={{ background: "var(--gradient-brand)" }} />
                      <h3 className="text-2xl font-extrabold tracking-tight text-heading">{section.category.name}</h3>
                      <span className="px-3 py-1 text-xs font-bold border rounded-full text-muted bg-white/50 border-border/40">{section.total} courses</span>
                    </div>
                    <button onClick={() => navigate(`${ROUTES.COURSES}?category=${section.category._id}`)} className="flex items-center gap-1.5 text-sm font-bold text-primary hover:gap-3 transition-all duration-200">
                      View all <Icon name="chevronRight" size={16} color="var(--color-primary)" />
                    </button>
                  </div>
                  <div className="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-4">
                    {section.courses.map((course, i) => (
                      <motion.div key={course._id} initial={{ opacity: 0, y: 16 }} whileInView={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.08 }}>
                        <CourseCard course={course} onEnroll={handleEnroll} onWishlist={handleWishlist} isEnrolled={enrolledCourseIds.includes(course._id)} isWishlisted={wishlistIds.includes(course._id)} />
                      </motion.div>
                    ))}
                  </div>
                </motion.div>
          ...
          <div className="mt-12 text-center">
            <button className="btn-aurora-outline" onClick={() => navigate(ROUTES.COURSES)}>Browse All Courses</button>
          </div>
        </section>
        ...
        <section className="px-6 py-16 mx-auto max-w-7xl">
          ...
          >
            ...
            <button
              ...
              className="hidden sm:flex items-center gap-2 text-sm font-bold text-primary hover:gap-3 transition-all duration-200"
          ...
          {latestBlogs.length > 0 && (
            ...
            >
              <div>
                <h4 style={{ color: "white", fontWeight: 800, fontSize: 18, margin: 0, letterSpacing: "-0.01em" }}>
                  Want to share your knowledge?
                </h4>
                <p style={{ color: "rgba(255,255,255,0.8)", fontSize: 13, margin: "4px 0 0", fontWeight: 500 }}>
                  Become an instructor and publish your own articles to the community.
                </p>
              </div>
              <button
        ...
        <section className="px-6 py-16 pb-24 mx-auto text-center max-w-7xl">
          <h2 className="mb-3 text-4xl font-extrabold tracking-tight text-heading">Trusted by Thousands</h2>
          <p className="mb-16 font-medium text-muted">The fastest-growing tech learning community in Southeast Asia</p>
          <div className="grid grid-cols-2 gap-8 lg:grid-cols-4">
            {STATS.map((s) => (
              <motion.div key={s.label} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} transition={{ delay: s.delay }}>
                <div className="deep-glass p-12 rounded-[3rem] relative overflow-hidden flex flex-col justify-center items-center">
                  <h3 className="mb-4 text-6xl font-black gradient-text" style={{ letterSpacing: "-0.05em" }}>{s.value}</h3>
